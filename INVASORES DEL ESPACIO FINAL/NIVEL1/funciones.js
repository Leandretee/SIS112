...
function Enemigo(x, y) {
    ...
    this.w = 35;
    this.veces = 0;
    this.dx = 2; // Velocidad horizontal más lenta
    this.ciclos = 0;
    this.num = 20; // Más ciclos antes de cambiar de dirección
    this.figura = true;
    this.vive = true;

    this.dibuja = function () {
        if (this.ciclos > 30) { // Retraso aumentado
            if (this.veces > this.num) {
                this.dx *= -1;
                this.veces = 0;
                this.num = 30; // Incrementar para un movimiento aún más lento
                this.y += 40; // Bajar una fila
            } else {
                this.x += this.dx;
            }
            this.veces++;
            this.ciclos = 0;
            this.figura = !this.figura; // Alternar sprite
        } else {
            this.ciclos++;
        }
...
function disparaEnemigo() {
    ...
    clearTimeout(disparoEnemigo);
    disparoEnemigo = setTimeout(disparaEnemigo, tiempoDisparo); // Nuevo tiempo de disparo
}
